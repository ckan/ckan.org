...
class FeaturesPage(MetadataPageMixin, Page):
    ...
    content_panels = Page.content_panels + [
        ...
        MultiFieldPanel(
            [
                InlinePanel('general_features', label="General Feature", min_num=2, max_num=4),
            ],
...
class CkanForPage(MetadataPageMixin, Page):
    ...
    def get_context(self,request, *args, **kwargs):
        context = super().get_context(request, *args, **kwargs)
        context['blog_posts'] = BlogPostPage.objects.filter(
            tags__name__in=[self.page_for.capitalize()]).live(
                ).public().order_by('-created')[:2]
        flt = 'government' if self.page_for == 'enterprise' else 'enterprise'
        bottom_block_page = CkanForPage.objects.live(
            ).public().filter(page_for=flt)
        context['bottom_block_page'] = bottom_block_page[0] if bottom_block_page else None

    ...
    def handle(self, *args, **options):
        try:
            ...
            with open(default_image_path, "rb") as imagefile:
                default_image = Image(file=ImageFile(BytesIO(imagefile.read())),
                    title=ntpath.basename(imagefile.name), width=520, height=390)
        ...
        with open('blog_posts.json', 'r') as f:
            ...
            for page in data:
                ...
                created = datetime.strptime(
                    page.get('post_date'), '%Y-%m-%d %H:%M:%S')
